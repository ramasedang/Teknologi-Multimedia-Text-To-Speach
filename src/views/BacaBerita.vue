<template >
    <div>
        <div class="card">
            ...
            <div>
                <button class="btn btn-danger" onclick=" window.open('https://www.cnnindonesia.com/')"> CNN
                    INDONESIA</button>
            </div>
            <div class="mt-2">
                <button class="btn btn-success" @click="getBeritaNasional"> Berita Nasional Terkini</button>
            </div>
            <div class="mt-2">
                <button class="btn btn-success" @click="getBeritaInternasional"> Berita Internasional Terkini</button>
            </div>
            <!-- Text Area  for the User to Type -->
            <textarea class="form-control mt-3" v-model="textInput" cols="30" rows="10"
                placeholder="Masukan link...  contoh: https://www.cnnindonesia.com/nasional/20230316200836-12-926104/kompolnas-kalau-lebih-banyak-mudarat-pelat-rf-disetop-saja"></textarea>
...
export default {
    ...
    methods: {
        async get_sound(url) {
            ...
            await axios.get('http://8.219.195.118:8081/cnn?url_cnn=' + this.textInput)
                .then(response => {
        ...
        async getBeritaNasional() {
            ...
            let berita;
            await axios.get('http://8.219.195.118:5000/nasional').then(response => {
                berita = response.data.data;
                
                let berita_nasional = berita[0]
                // console.log(berita_nasional);
                this.textInput = berita_nasional.link;
                Swal.close();
            }).catch(error => {
                console.log(error);
            }).finally(() => {
        ...
        async getBeritaInternasional() {
            ...
            let berita;
            await axios.get('http://8.219.195.118:5000/internasional').then(response => {
                berita = response.data.data;
